...
PARAMETER_MAX          = [ 0.20,  0.50,  0.020,       8,    10,    20,    1.0,  1.0,    15,      10,    10,  0.08,       5]
PARAMETER_MIN          = [-0.20, -0.50, -0.160,      -8,   -10,   -20,   -1.0, -1.0,   -15,     -10,   -10,  0.03,       0]
...
def get_arm_joint_targets(t_global,
    ...
    q_arm[0] += amp * np.sin(phase) + 0.4
    q_arm[3] += amp * np.sin(phase) - 0.4
...
def get_arm_joint_targets_from_step(leg_st,
    ...
    if leg_st == +1:
        gait_phase = step_phase
    else:
        gait_phase = step_phase + 0.5

    phase = 2.0 * np.pi * gait_phase

    q_arm = np.array(arm_nominal, dtype=float)

    q_arm[0] += shoulder_offset + amp * np.sin(phase)
    q_arm[3] += -shoulder_offset + amp * np.sin(phase)
